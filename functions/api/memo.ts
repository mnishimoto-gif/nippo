...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  const row = await context.env.DB.prepare("SELECT content, updated_at as updatedAt FROM memo WHERE id = 1").first<MemoRow>();
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  await context.env.DB.prepare("UPDATE memo SET content = ?, updated_at = ? WHERE id = 1")
